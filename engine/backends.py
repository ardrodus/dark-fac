"""CodergenBackend implementations that bridge the pipeline to LLM backends.

AgentLoopBackend: Wraps a coding agent Session as a CodergenBackend.
    Pipeline node -> Agent Session -> Client -> LLM

DirectLLMBackend: Calls the Client directly (no agent loop).
    Pipeline node -> Client -> LLM
...
from dark_factory.engine.agent.prompt_layer import layer_prompt_for_node
from dark_factory.engine.agent.registry import resolve_security_policy
...
from dark_factory.engine.types import Client, Message, Request
...
class AgentLoopBackend:
    ...
    async def run(
        ...
        # Resolve agent_type from node role attribute (used by protocol layer)
        agent_type = node.attrs.get("role", "")
        ...
        # Resolve per-node security policy and wire it + workspace root
        # into the tool registry so tool calls are confined per role.
        policy = resolve_security_policy(node.id, node.attrs)

        # Confine all file tools to workspace root (not global filesystem)
        if self._workspace_root is not None:
            set_allowed_roots([self._workspace_root])
        ...
        try:
            async with Session(
                ...
                # Wire security policy and workspace root into tool registry
                session.tool_registry.security_policy = policy
                if self._workspace_root is not None:
                    session.tool_registry.workspace_root = self._workspace_root
