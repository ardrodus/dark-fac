...
def truncate_output(
    ...
    if len(lines) > limits.max_lines:
        head_line_count = int(limits.max_lines * limits.head_ratio)
        tail_line_count = limits.max_lines - head_line_count
        head_part = lines[:head_line_count]
        tail_part = lines[-tail_line_count:] if tail_line_count > 0 else []
        omitted_lines = len(lines) - head_line_count - tail_line_count
        output = (
            "\n".join(head_part)
