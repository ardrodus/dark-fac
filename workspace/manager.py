...
# Auto-generated by Dark Factory workspace bootstrap
...
def _bootstrap_ralph_tui(ws_path: Path) -> None:
    """Create ``.ralph-tui/config.toml`` so ralph-tui --headless works."""
    config_dir = ws_path / ".ralph-tui"
    config_file = config_dir / "config.toml"
    if config_file.exists():
        return
    try:
        config_dir.mkdir(parents=True, exist_ok=True)
        config_file.write_text(_RALPH_TUI_CONFIG, encoding="utf-8")
        logger.info("Bootstrapped ralph-tui config at %s", config_file)
    except OSError:
        logger.debug("Could not bootstrap ralph-tui config", exc_info=True)
...
def create_workspace(
    ...
    *,
    ...
    # Ensure ralph-tui can run headless in this workspace
    _bootstrap_ralph_tui(ws_path)
