...
def _apply_patches(stack: ExitStack, overrides: dict, captured: list[str]) -> None:
    ...
    defaults = {
        ...
        "bootstrap": MagicMock(success=True, runtime_ok=True, env_created=True, deps_installed=True, env_path=".venv", errors=()),
    ...
    p(patch("dark_factory.setup.orchestrator._run_project_analysis", return_value=defaults["analysis"]))
    ...
    p(patch("dark_factory.setup.orchestrator._bootstrap_workspace_env", return_value=defaults["bootstrap"]))
    ...
    fake_ws = MagicMock(success=True, workspace=MagicMock(path="/tmp/df-test"))
...
class TestZeroDependenciesFound:
    ...
    def test_all_deps_missing(self) -> None:
        ...
        assert "FAILED" in full_output or "failed" in full_output.lower() or "✘" in full_output
...
class TestAllDependenciesFound:
    ...
    def test_all_deps_found(self) -> None:
        ...
        full_output = "".join(captured)
        assert exit_code == 0
        assert "PASSED" in full_output or "passed" in full_output.lower() or "✔" in full_output
        assert "FAILED" not in full_output
...
class TestPartialPhaseFailure:
    ...
    def test_partial_failure_exit_code_1(self) -> None:
        ...
        fail_ws = MagicMock(success=False, workspace=None, message="clone failed")

        captured: list[str] = []
        with ExitStack() as stack:
            p = stack.enter_context
            p(patch("dark_factory.integrations.shell.gh", return_value=ok_result))
            ...
            p(patch("dark_factory.core.config_manager.resolve_config_dir", return_value=Path("/tmp/.df")))
            p(patch("dark_factory.workspace.manager.create_workspace", return_value=fail_ws))
            p(patch("sys.stdout.write", side_effect=lambda s: captured.append(s)))
