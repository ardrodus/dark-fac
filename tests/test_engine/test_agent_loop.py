...
class TestSession:
    ...
    def test_tools_registered_via_constructor(self):
        ...
        # Factory's SubagentManager.create_interactive_tools() returns [] (stub),
        # so only ALL_CORE_TOOLS are registered.
        assert len(session.tool_registry.definitions()) >= len(ALL_CORE_TOOLS)
